...
namespace UNO.MVCApp.Controllers
{
    ...
    public class EmployeeHierarchyController : Controller
    {
        ...
        //
        // GET: /EntEmployee/
        [AccessCheck(IdParamName = "EmployeeHierarchy/index")]
        public async Task<ActionResult> Index()
        {
            ...
            return await GetEmployeeHierarchyList();
        }

        private async Task<ActionResult> GetEmployeeHierarchyList()
        {
            using (HttpClient client = new HttpClient())
            {
                client.AddTokenToHeader("");
                uri = string.Format("{0}employeehierarchy", _uri);

                var result = await client.GetAsync(uri);

                var employeeHierarchy = await result.Content.ReadAsAsync<List<EmployeeHierarchyInfo>>();
                return View(employeeHierarchy);
            }
        }
        //
        ...
        {
            EmployeeHierarchy employee = await GetEmployee(id);
            await BindDropDown();
        ...
        public async Task<ActionResult> Create(EmployeeHierarchyCreateModel collection)
        {
        ...
        private async Task<EmployeeHierarchy> GetEmployee(int id)
        {
            EmployeeHierarchy employee;
            using (HttpClient client = new HttpClient())
            {
                uri = string.Format("{0}employeehierarchy/{1}", _uri, id);

                var result = await client.GetAsync(uri);

                employee = await result.Content.ReadAsAsync<EmployeeHierarchy>();
            }
            return employee;
        }
